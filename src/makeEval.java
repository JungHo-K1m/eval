...
public class makeEval {
    public static double eval(final String str){
        ...
        Stack<Character> operations = new Stack<Character>();   //연산자를 넣을 스택

        //입력한 문자열 길이만큰 반복
        for(int i=0; i < str.length(); i++){
            if(token[i] == ' ') continue;   //빈칸(스페이스바)는 건너뛰자
            ...
            //읽은 문자가 닫는 괄호인 경우
                ...
                while (operations.peek() != '(')    //스택의 마지막에 들어간 값이 시작 괄호인지 확인
            ...
            //토큰의 i번째 문자가 연산자인 경우
                ...
                //연산자 스택이 비어있지 않고, 연산 우선순위가 true인 경우
                while (!operations.empty() && hasPrecedence(token[i], operations.peek()))
                    values.push(applyOp(operations.pop(), values.pop(), values.pop())); //값을 계산하여 숫자 스택에 넣는다
                operations.push(token[i]);  //연산자 스택에 넣는다
            }
        }
